...
export function launch(launchFunction) {
	...
	try {
		...
		if (!window.rocket.payload.some(satellite => satellite.type === 'NFSAT')) {
			throw new Error('NFSAT payload was not put into orbit');
		}

		if (!window.rocket.payload.some(satellite => satellite.type === 'FISHSAT')) {
		...
		const state = logTry(
			window.rocket.cost +
				window.rocket.payload.reduce((acc, satellite) => acc + satellite.cost, 0),
		);
